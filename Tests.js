...
const Tests = ({ jobsList, jobEdit, isAdmin }) => {
  ...
  useEffect(() => {
    ...
    if (data.length === 0) {
      ...
      setJob(data[0]);
      const dataN = data[0].applications
        .filter((app) => app.test)
        .map((app) => ({ ...app, checked: false }));
      setApplications(dataN);
      setFilteredData(
        dataN.filter(
          (job) =>
            job.test.status.includes(filters.status) &&
            `${job.user.firstName.toLowerCase()} ${job.user.lastName.toLowerCase()}`.includes(
    ...
    setFilteredData(
      filteredData.map((application) => {
        return {
          ...application,
          checked:
            application.status === "2" && application.test.status === "2"
              ? !selectAll
              : false,
        };
      })
  ...
  const handleScheduleInterview = () => {
    const applicationsIds = filteredData
        ...
        <PortletHeader
          ...
          toolbar={
            <PortletHeaderToolbar>
              {isAdmin && (
                <button
                  className="btn btn-label btn-bold btn-sm"
                  onClick={handleShow}
                  disabled={
                    filteredData.filter((app) => app.checked).length === 0
                  }
                >
          ...
          <Table responsive className="mt-3">
                ...
                {isAdmin && (
                  <th>
                    <input
                      ...
                      disabled={
                        filteredData.filter((app) => app.test.status === "2")
                          .length === 0
                      }
                      checked={selectAll}
                      onChange={onCheckAll}
            ...
            <tbody>
              {filteredData.length === 0 ? (
                <tr>
                  <td colSpan={isAdmin ? 8 : 5} style={{ textAlign: "center" }}>
                    No Tests Found
                  </td>
                </tr>
              ) : (
                filteredData
                  ...
                  .map((application, i) => (
                    <tr key={i}>
                      {isAdmin && (
                        <td>
                          <input
                            ...
                            className="form-check"
                            disabled={
                              application.test.status !== "2" ||
                              parseInt(application.status) > 2
                            }
                      ...
                      {isAdmin && (
                        <td>
                          {application.user.user_details ? (
                            <a
                              href={`/pdf/${application.user.user_details.cv.filename}`}
                              target="_blank"
                      ...
                      <td>
                        {moment(application.test.date).format("DD/MM/YYYY")}
                      </td>
                      {isAdmin && (
                        <td>
                          <Tooltip title="Mark as Passed!" placement="top">
                            <span>
                              <button
                                className="btn btn-icon h-auto w-auto"
                                disabled={parseInt(application.status) > 2}
                                onClick={() =>
                                  handleShowStatus(
                                    application._id,
                                    "2",
